...
export default function ColorPicker({colorData, onUpdateColor, onUpdateColorId}) {
    return (
        <View className="w-full flex-col mt-2.5">
            ...
            <ScrollView contentContainerStyle={{opacity:1}} horizontal={true} showsHorizontalScrollIndicator={false}>
                {colorData.map((item, index) => (
                    <WrappedColor key={index} colorHex={item.code} colorId={item._id} updateColor={onUpdateColor} updateColorId={onUpdateColorId}/>
                ))}
            </ScrollView>
        </View>
    )
