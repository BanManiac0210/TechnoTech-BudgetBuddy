...
export default function AddInitMoneySourceScreen({navigation}) {
  ...
  const [defaultIconId, setDefaultIconId] = useState("6661dd0be3f66c3a5cbd97eb");
  ...
  const createNewMS = async () => {
    try {
      ...
      console.log("Failed to create new MS: ", error);
    }
  }
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try{
        ...
        console.log("Failed to fetch data on AddMoneySourceScreen: ", error);
      } 
    }
      ...
      <View className="w-screen h-screen bg-white items-center justify-center px-[20] pt-[50] pb-[100]">
        ...
        >
          <View className="w-full flex-row space-x-3 items-center">
            <Text className="flex-1 font-bold text-xl text-purple-900">Tạo nguồn tiền mới</Text>  
          </View>
